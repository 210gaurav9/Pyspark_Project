...
#Find out the customer total purchase of every month
#write the data to mysql table

def sales_mart_sql_transformation_write(final_sales_team_data_mart_df):
    ...
    final_sales_team_data_mart = final_sales_team_data_mart_df\
        ...
        .select("store_id","sales_person_id",
                concat(col("sales_person_first_name"), lit(" "),
                       col("sales_person_last_name")).alias("first_name"),
    ...
    #Write the data into mysql customers data mart table
    db_writer = DatabaseWriter(config.url,config.properties)
    db_writer.write_dataframe(final_sales_team_data_mart_table,config.sales_team_data_mart)